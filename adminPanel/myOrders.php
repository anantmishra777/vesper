<?php
	...
	$query1 = $base->select('clients', '*', array('c_clientID'=> $_SESSION['clientID']));
	if($query1->c_role==2)
	{
		header('Location: ../logout');
		die();
										...
										<tbody>
											...
											<?php
												if($query1->c_is_parent==1)
													$clientID = $query1->c_parentID;
												else
													$clientID = $_SESSION['clientID'];

												try
												{
													$query = $conn2->prepare('SELECT * FROM orders WHERE c_clientID=:c_clientID ORDER BY o_orderDateTime DESC');
												...
												for($i=0; $i<$query->rowCount(); $i++)
												{
													...
													<tr>
														<td>
															<?php 
																if( $row[$i]['o_orderID']!='' )
																	echo htmlspecialchars($row[$i]['o_orderID']); 
															?>
														</td>

														<td>
															<?php 
																if( $row[$i]['o_formLevel'] < 4 )
																	echo 'Sverige'; 
																else
																	echo 'Utomlands';
															?>
														...
														<td>
															<?php 
																if( $row[$i]['o_formLevel']<4 )
																	echo htmlspecialchars($row[$i]['o_formLevel']); 
															?>
														</td>
														...
														<td>
															<?php 
																if( $row[$i]['o_name']!='' )
																	echo htmlspecialchars($row[$i]['o_name']); 
															?>
														</td>

														<td >
															<?php 
																if( $row[$i]['o_personnummer']!='' )
																	echo htmlspecialchars($row[$i]['o_personnummer']); 
															?>
														</td>


														<td >
															<?php echo htmlspecialchars($row[$i]['o_request_description']); ?>
														</td>

														<td >
															<?php echo htmlspecialchars($row[$i]['o_contact_number']); ?>
														</td>

														<td >
															<?php echo htmlspecialchars($row[$i]['o_email']); ?>
														</td>

														<td>
															<?php echo htmlspecialchars(substr($row[$i]['o_orderDateTime'], 0, strpos($row[$i]['o_orderDateTime'], ' '))); ?>
														</td>

														<td >
															<?php
																switch( $row[$i]['o_orderStatus'] )
																{
																	case 1:
														...
														<td>
															<?php
																if($row[$i]['r_reportLocation']!='')
																{
																	?>
																	<a href="download.php?url=<?php echo htmlspecialchars($row[$i]['r_reportLocation']); ?>">Download</a>
																	<?php
																}	
															?>
														</td>
