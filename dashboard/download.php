<?php
    ...
    if( htmlspecialchars($_GET['url']) )
    {
        ...
        $insert_log = array('l_user' => $base->enc($_SESSION['email']),
                            'l_logDescription' => $base->enc('File downloaded by '.$_SESSION['email'].PHP_EOL.'(File URL: '.htmlspecialchars($_GET['url']).')'),
                            'l_logType' => $base->enc('download_file'),
                            'l_accessLevel' => $base->enc(0),
                            'l_sourceFile' => $base->enc(BASE_URL.'/dashboard/myOrders.php'),
